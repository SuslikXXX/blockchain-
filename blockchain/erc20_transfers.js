...
async function simpleTransfer(token, from, to, amount) {
  ...
  console.log(`Amount: ${hre.ethers.formatEther(amount)} TST`);
  
  const tx = await token.connect(from).transfer(to.address, amount);
  await tx.wait();
  
  const balance = await token.balanceOf(to.address);
  console.log(`New balance of ${to.address}: ${hre.ethers.formatEther(balance)} TST`);
...
async function batchTransfers(token, from, recipients, amounts) {
  ...
  for (let i = 0; i < recipients.length; i++) {
    console.log(`Transfer ${i + 1}: ${hre.ethers.formatEther(amounts[i])} TST to ${recipients[i].address}`);
...
async function approveAndTransferFrom(token, owner, spender, recipient, amount) {
  ...
  console.log(`Approved ${hre.ethers.formatEther(amount)} TST for spender`);
  
  // Check allowance
  const allowance = await token.allowance(owner.address, spender.address);
  console.log(`Allowance: ${hre.ethers.formatEther(allowance)} TST`);
  
  // TransferFrom
  const transferTx = await token.connect(spender).transferFrom(owner.address, recipient.address, amount);
  await transferTx.wait();
  console.log(`Transferred ${hre.ethers.formatEther(amount)} TST from ${owner.address} to ${recipient.address}`);
  
  const balance = await token.balanceOf(recipient.address);
  console.log(`New balance of ${recipient.address}: ${hre.ethers.formatEther(balance)} TST`);
...
async function multipleApprovals(token, owner, spenders, amounts) {
  ...
  for (let i = 0; i < spenders.length; i++) {
    console.log(`Approving ${hre.ethers.formatEther(amounts[i])} TST for ${spenders[i].address}`);
...
async function checkBalances(token, addresses) {
  ...
  for (const address of addresses) {
    ...
    console.log(`${address}: ${hre.ethers.formatEther(balance)} TST`);
  }
}
...
async function main() {
  ...
  // Simple transfer
  await simpleTransfer(token, owner, addr1, hre.ethers.parseEther("1000"));
